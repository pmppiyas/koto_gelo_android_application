...
import { groupService, Group, CreateGroupPayload } from '../../services/groupService';
...
export const EditGroupModal: React.FC<EditGroupModalProps> = ({
  ...
  useEffect(() => {
    if (group) {
      ...
      setSelectedType(group.type || 'OTHER');
      setError('');
    }
  }, [group]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to update group. Please check connection.');
    } finally {
          ...
          <View className="bg-card rounded-t-3xl p-5 max-h-[85%] border-t border-border shadow-2xl">
            <View className="flex-row items-center justify-between pb-3 border-b border-border">
              <View className="flex-row items-center gap-2.5">
                ...
                <Text className="text-lg font-bold text-foreground">Edit Group Details</Text>
              </View>
              <TouchableOpacity onPress={handleClose} className="p-1" activeOpacity={0.7}>
